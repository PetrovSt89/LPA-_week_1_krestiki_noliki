...
# функция хода компьютера
def comp_step(base_turns):
    steps = [step for step in ALL_STEPS if step not in base_turns]
    turn = random.choice(steps)
    clear()
    return turn
...
def check_win(field):
    ...
    # основные проверки
    sum_field = field + field_reverce(field) + diag(field) + revers_diag(field)
...
def game_progress():
    ...
    step = 'man' if x_o_man == 'x' else 'comp'
    while True:
        if step == 'man':    
        ...
        if sorted(ALL_STEPS) == sorted(base_turns):
            clear()
            return ''
